...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'addTerm') {
    $response = $academicPeriodController->addAcademicYearWithSemesters();
    $status = $response[0];  // 'success' or 'error'
    $message = $response[1]; // Message for the user
}
                ...
                <div class="bg-white rounded p-3 shadow-sm border">
                    ...
                    <section>
                        ...
                        <div class="d-flex gap-2 pt-2 ">
                            <h6>Academic Year:</h6>
                            <p><?php echo isset($currentTerm['academic_year']) ? htmlspecialchars($currentTerm['academic_year']) : 'N/A'; ?>
                            </p>
                        </div>
                                ...
                                <tbody>
                                    <?php if (is_array($allTerms) && !empty($allTerms)): ?>
                                        <?php foreach ($allTerms as $term): ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($term['academic_year'] ?? 'N/A'); ?></td>
                                                <td>
                                                    <?php
